...
tail(arrange(assistant_smaller, endowment))

head(arrange(assistant_smaller, endowment))
...
**Answer:** For every order of magnitude change in endowment, faculty salaries are predicted to go up by `r coef(lm_fit)[2]`
...
**Answers**
SSModel = 126025255258 
SSError = 175847574704 
...
plot(assistant_smaller$log_endowment, 
     assistant_smaller$salary_tot,
...
for (i in 1:100){
  ...
  boot_sample <- sample_n(assistant_smaller, size = n_cases, replace = TRUE)
